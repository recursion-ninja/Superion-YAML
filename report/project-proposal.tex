...
Some problems we experienced so far was sampling bias during input generation. We also noticed that the input mutation of parse trees performed small-scale changes on the local level. To fix some of these issues, we plan on restructuring the input generation rules, such as placing constraints on transition rules in the CFG. We will also utilize mutation operators, such as random mutation, random recursive, and splicing operators.
