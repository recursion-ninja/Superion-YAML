...
However, these fuzzers do not support the file formats which we are sampling.
...
We will discuss the difficulties presented by each tool, our attempt in adapting our methodology to address these difficulties, along with the CVE replication, which was achievable.
...
Fuzzing tools generate input, which is directed towards the program under test and detects whether the input was accepted or caused unexpected behavior.
...
There are also two broad classes of input generations; naive and structured\cite{fuzzingsurvey} inputs.
...
With the Grimoire inaccessible as a baseline metric, we substituted AFL and AFL++ to be used as our baseline metrics in anticipation of later success extending Gramatron and Superion.
When extension of Gramatron and Superion became exceedingly improbable within our time constraints, we finalized our methodology to utilize AFL as the baseline measurement and AFL++ as the structured-aware fuzzer. These fuzzers were utilized on fuzzgoat before running the fuzzers on YAML, JSON, and Markdown. Fuzzgoat is a vulnerable C program used for testing of fuzzers.
...
Quantitatively, we measured the execution time of AFL and AFL++ and compare their inputs generated per second.
...
Gramatron's approach to fuzzing described above in \emph{Motivation}, while novel and efficient, is not without its limitations.
...
\hrule height 0.4mm
\begingroup \fontsize{10pt}{10pt} \selectfont 
\verbatiminput{JSONsource_automata.json}
\vspace{2mm}
\endgroup \hrule height 0.4mm
\vspace{3mm}
...
\begin{alltt}
\verbatiminput{MarkdownParser.g4}
\end{alltt} \vspace{-6mm} \endgroup \hrule height 0.4mm
\vspace{10mm}
\centerline{\textbf{Figure 5.} Excerpts of Markdown grammar in \texttt{.g4} format}
\vspace{10mm}

Our assessment of Superion is limited, as we were not able to successfully extend its support for any of the attempted grammars.
...
Ultimately Superion promises high fidelity linguistic coverage in it's structure-aware fuzzing, but we were unable to successfully extend and measure these promises.
...
\vspace{4mm}\\ \textbf{Table 1.} Target CVEs for each languages
\end{table}
...
\vspace{4mm}
\hrule height 0.4mm
\begingroup \fontsize{11pt}{11pt} \selectfont \begin{alltt}
\input{input.js}
\end{alltt} \vspace{-6mm} \endgroup \hrule height 0.4mm
\vspace{6mm}
...
\vspace{1mm}
\begin{center}
	\includegraphics[scale=0.5, trim={0 0 7mm 0}, clip]{afl_yaml_cpp_3hrs.jpg}\\
...
\vspace{1mm}
	\includegraphics[scale=0.7, trim={0 0 7mm 0}, clip]{yamlfuzzer.png}\\
...
In order to analyze these crashes files, we've tried to reproduce the known CVEs by understanding how the specific CVE is found from those discoveries.
...
\vspace{3mm}
\begin{center}
	\includegraphics[scale=1, trim={0 0 0 0}, clip]{yaml-sample-crashes.png}\\
	\textbf{Figure 12.} Crashes testing for YAML-CPP with AFL
\end{center}
\vspace{3mm}

\begin{center}
	\includegraphics[scale=0.35, trim={0 3cm 0 0}, clip]{yaml_cpp_crashes.png}\\
...
\begin{tabular}{V{4}l|l|lV{4}} \hlineB{3}
	...
	\multicolumn{1}{c|}{175} &
	\multicolumn{1}{cV{4}}{535} \\ \hline
	\multicolumn{1}{V{4}c|}{fuzzgoat} & 
	\multicolumn{1}{c|}{38} &
	\multicolumn{1}{cV{4}}{20} \\ \hlineB{3}
\end{tabular}
}
...
\begin{tabular}{V{4}l|l|lV{4}} \hlineB{3}
	\multicolumn{1}{V{4}c|}{} & \multicolumn{1}{c|}{AFL} & \multicolumn{1}{cV{4}}{AFL++}  \\ \hline
	\multicolumn{1}{V{4}c|}{\texttt{yaml-cpp}} & 
	\multicolumn{1}{c|}{1298/sec 1cycle} &
	\multicolumn{1}{cV{4}}{2191/sec 1cycle} \\ \hline
	\multicolumn{1}{V{4}c|}{md4c} & 
	\multicolumn{1}{c|}{2855/sec 0cycle} &
	\multicolumn{1}{cV{4}}{2198/sec 2cycle} \\ \hline
	\multicolumn{1}{V{4}c|}{fuzzgoat} & 
	\multicolumn{1}{c|}{2169/sec 7cycle} &
	\multicolumn{1}{cV{4}}{347.8/sec 3cycle} \\ \hlineB{3}
\end{tabular}
}
...
\begin{tabular}{V{4}l|l|lV{4}} \hlineB{3}
	...
	\multicolumn{1}{cV{4}}{5404} \\ \hline
	\multicolumn{1}{V{4}c|}{fuzzgoat} & 
	\multicolumn{1}{c|}{651} &
	\multicolumn{1}{cV{4}}{564} \\ \hlineB{3}
\end{tabular}
}
...
\item \texttt{2021-10-24:} Superion \& Gramatron extension; Grimoire setup
\item \texttt{2021-10-31:} Superion \& Gramatron toils; more Grimoire setup
\item \texttt{2021-11-07:} Superion \& Gramatron toils; more Grimoire setup
\item \texttt{2021-11-14:} Superion \& Gramatron toils; Abandon Grimoire use AFL \& AFL++
\item \texttt{2021-11-21:} Superion \& Gramatron toils; Setup AFL \& AFL++ 
\item \texttt{2021-11-28:} Failed Superion \& Gramatron extension; use AFL++
